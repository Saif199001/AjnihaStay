...
def validate_owner_membership_invariant(apps, schema_editor):
    ...
    for workspace in Workspace.objects.order_by("pk").iterator():
        matching = Membership.objects.filter(
            workspace_id=workspace.pk, user_id=workspace.owner_id,
            role="owner", is_active=True,
        ).count()
        active_owners = Membership.objects.filter(
            workspace_id=workspace.pk, role="owner", is_active=True
        ).count()
        if matching != 1 or active_owners != 1:
...
BEGIN
    ...
    IF v_owner_id IS NULL THEN
        RAISE EXCEPTION 'Workspace % must have an owner', v_workspace_id
            USING ERRCODE = '23514';
    END IF;

    SELECT COUNT(*) INTO active_owner_count
    FROM workspaces_membership
    WHERE workspaces_membership.workspace_id = v_workspace_id
      AND role = 'owner' AND is_active = TRUE;

    SELECT COUNT(*) INTO matching_owner_count
    FROM workspaces_membership
    WHERE workspaces_membership.workspace_id = v_workspace_id
      AND user_id = v_owner_id AND role = 'owner' AND is_active = TRUE;
    ...
    IF active_owner_count <> 1 OR matching_owner_count <> 1 THEN
        RAISE EXCEPTION
            ...
            v_workspace_id USING ERRCODE = '23514';
    END IF;
    ...
    IF TG_TABLE_NAME = 'workspaces_membership'
       ...
       AND OLD.workspace_id IS DISTINCT FROM NEW.workspace_id THEN
        SELECT owner_id INTO v_owner_id
        FROM workspaces_workspace WHERE id = OLD.workspace_id;

        SELECT COUNT(*) INTO active_owner_count
        FROM workspaces_membership
        WHERE workspaces_membership.workspace_id = OLD.workspace_id
          AND role = 'owner' AND is_active = TRUE;
        ...
        IF active_owner_count <> 1 OR matching_owner_count <> 1 THEN
            RAISE EXCEPTION
                'Workspace % must have exactly one active owner membership matching Workspace.owner',
                OLD.workspace_id USING ERRCODE = '23514';
        END IF;
    END IF;
...
class Migration(migrations.Migration):
    dependencies = [("workspaces", "0007_membership_viewer_role")]
    operations = [
        migrations.RunPython(validate_owner_membership_invariant, migrations.RunPython.noop),
        migrations.AddConstraint(
            model_name="membership",
            constraint=models.UniqueConstraint(
                fields=["workspace"], condition=Q(role="owner", is_active=True),
                name="unique_active_workspace_owner",
            ),
        ),
        migrations.RunSQL(TRIGGER_FUNCTION_SQL + TRIGGER_CREATE_SQL, reverse_sql=TRIGGER_DROP_SQL),
    ]
