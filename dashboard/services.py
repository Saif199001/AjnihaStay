...
from payments.late_fee_models import LateFee
...
def _current_occupancy_queryset(today):
    return Occupancy.objects.filter(is_active=True, check_in_date__lte=today).filter(Q(check_out_date__isnull=True) | Q(check_out_date__gte=today))
...
def _canonical_workspace_financial_rows(workspace):
    ...
    adjustments = {}
    for row in workspace.financial_adjustments.filter(invoice_id__in=invoice_ids).values("invoice_id", "adjustment_type").annotate(total=Sum("amount")):
        adjustments.setdefault(row["invoice_id"], {})[row["adjustment_type"]] = row["total"] or Decimal("0")

    allocations = {
        row["invoice_id"]: row["total"] or Decimal("0")
        for row in PaymentAllocation.objects.filter(payment__workspace=workspace, invoice_id__in=invoice_ids).values("invoice_id").annotate(total=Sum("amount"))
    }
    advance_applications = {
        row["invoice_id"]: row["total"] or Decimal("0")
        for row in AdvanceCreditApplication.objects.filter(invoice_id__in=invoice_ids).values("invoice_id").annotate(total=Sum("amount"))
    }
    late_fees = {
        row["invoice_id"]: row["total"] or Decimal("0")
        for row in LateFee.objects.filter(workspace=workspace, invoice_id__in=invoice_ids).values("invoice_id").annotate(total=Sum("amount"))
    }
    ...
    for invoice in invoices:
        ...
        reducing = sum((by_type.get(kind, Decimal("0")) for kind in reducing_types), Decimal("0"))
        gross = invoice.total_amount or Decimal("0")
        late_fee_total = late_fees.get(invoice.id, Decimal("0"))
        adjusted = gross + debit + late_fee_total - reducing
...
def get_dashboard_data(workspace, *, period_start=None, period_end=None, upcoming_days=30, availability_limit=None, upcoming_vacancy_limit=None):
    ...
    subunit_occupancy_qs = Prefetch("occupancies", queryset=current_occupancies, to_attr="dashboard_current_occupancies")
    active_subunits = SubUnit.objects.filter(is_active=True).prefetch_related(subunit_occupancy_qs).order_by("subunit_number", "id")
    active_units = (
        Unit.objects.filter(is_active=True, property__workspace=workspace, property__is_active=True)
        .prefetch_related(Prefetch("subunits", queryset=active_subunits, to_attr="dashboard_subunits"), Prefetch("occupancies", queryset=unit_occupancies, to_attr="dashboard_unit_occupancies"))
        ...
        for subunit in subunits:
            if getattr(subunit, "dashboard_current_occupancies", []):
                ...
                if len(available_spaces) < availability_limit:
                    available_spaces.append({"property_id": unit.property_id, "property_name": unit.property.name, "unit_id": unit.id, "unit_number": unit.unit_number, "subunit_id": subunit.id, "subunit_number": subunit.subunit_number, "available_capacity": 1, "type": "subunit"})
        unit_remaining = max(unit.capacity - len(unit.dashboard_unit_occupancies), 0)
        if unit_remaining:
            availability_total += 1
            if len(available_spaces) < availability_limit:
                available_spaces.append({"property_id": unit.property_id, "property_name": unit.property.name, "unit_id": unit.id, "unit_number": unit.unit_number, "subunit_id": None, "subunit_number": None, "available_capacity": unit_remaining, "type": "unit"})
    ...
    upcoming_queryset = current_occupancies.filter(check_out_date__isnull=False, check_out_date__gte=today, check_out_date__lte=upcoming_end, tenant__workspace=workspace).select_related("tenant", "unit__property", "subunit").order_by("check_out_date", "unit__property_id", "unit_id", "subunit_id", "id")
    for occupancy in upcoming_queryset:
        upcoming_vacancies_total += 1
        if len(upcoming_vacancies) >= upcoming_vacancy_limit:
            continue
        upcoming_vacancies.append({"occupancy_id": occupancy.id, "property_id": occupancy.unit.property_id, "property_name": occupancy.unit.property.name, "unit_id": occupancy.unit_id, "unit_number": occupancy.unit.unit_number, "subunit_id": occupancy.subunit_id, "subunit_number": occupancy.subunit.subunit_number if occupancy.subunit_id else None, "tenant_id": occupancy.tenant_id, "tenant_name": occupancy.tenant.full_name, "vacancy_date": occupancy.check_out_date})
    ...
    return {
        ...
        "summary": {"total_properties": total_properties, "total_units": total_units, "total_unit_capacity": total_unit_capacity, "occupied_unit_slots": occupied_unit_slots, "available_unit_slots": available_unit_slots, "occupancy_rate": round(occupancy_rate, 2), "active_tenants": active_tenants, "total_subunits": total_subunits, "occupied_subunits": occupied_subunits, "available_subunits": available_subunits},
        "financial": {"period_invoiced": period_invoiced, "period_rent": period_rent, "period_charges": period_charges, "period_collected": period_collected, "collection_rate": round(collection_rate, 2), "outstanding": outstanding, "overdue": overdue},
