...
from tenant.models import Tenant, Occupancy
...
class PaymentIntegrityTests(TestCase):
    ...
    def test_payment_cannot_overpay_invoice(self):
        create_payment(self.owner, self.payment_data("7000.00"))

        with self.assertRaises(ValidationError):
            create_payment(self.owner, self.payment_data("4000.00"))

    def test_payment_is_owner_scoped(self):
        with self.assertRaises(ValidationError):
            create_payment(self.other_owner, self.payment_data("1000.00"))
    ...
    def test_full_payment_marks_invoice_paid(self):
        create_payment(self.owner, self.payment_data("10000.00"))
        self.invoice.refresh_from_db()
