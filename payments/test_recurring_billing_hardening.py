...
from .recurring_billing_service import generate_due_recurring_billing, generate_recurring_billing_occurrence
...
class RecurringBillingHardeningTests(TestCase):
    def setUp(self):
        ...
        self.workspace = Workspace.objects.create(name="Recurring Billing Workspace", slug="recurring-billing-workspace", owner=self.owner)
        Membership.objects.create(workspace=self.workspace, user=self.owner, role="owner")
        property_obj = Property.objects.create(owner=self.owner, workspace=self.workspace, name="Recurring Property", property_type="pg", address="Delhi", city="Delhi", state="Delhi", pincode="110001")
        unit = Unit.objects.create(property=property_obj, unit_type="room", unit_number="RB-1", rent=Decimal("10000.00"))
        tenant = Tenant.objects.create(owner=self.owner, workspace=self.workspace, full_name="Recurring Tenant", phone="8888888888", permanent_address="Delhi")
    ...
    def test_monthly_anchor_does_not_drift_after_short_month(self):
        ...
        generate_recurring_billing_occurrence(self.owner, self.workspace, schedule, date(2026, 1, 31))
        schedule.refresh_from_db()
        self.assertEqual(schedule.next_run_date, date(2026, 2, 28))
        self.assertEqual(schedule.anchor_day, 31)

        generate_recurring_billing_occurrence(self.owner, self.workspace, schedule, date(2026, 2, 28))
        schedule.refresh_from_db()
    ...
    def test_occurrence_retry_is_idempotent_for_charge_and_invoice(self):
        ...
        first = generate_recurring_billing_occurrence(self.owner, self.workspace, schedule, date(2026, 1, 31))
        second = generate_recurring_billing_occurrence(self.owner, self.workspace, schedule, date(2026, 1, 31))
    ...
    def test_recurring_invoice_uses_specialized_canonical_ledger_transition(self):
        schedule = self.make_schedule()
        result = generate_recurring_billing_occurrence(self.owner, self.workspace, schedule, date(2026, 1, 31))
    ...
    def test_bounded_catch_up_processes_due_occurrences(self):
        ...
        results = generate_due_recurring_billing(self.owner, self.workspace, schedule, as_of_date=date(2026, 4, 1), catch_up=True, max_occurrences=3)
