...
def execute_transition(transition, *, user, workspace, **payload):
    ...
    Imports are intentionally lazy to avoid circular dependencies while the
    existing bounded services are incrementally migrated into this authority.
    """
    ...
    if transition is FinancialTransition.UPDATE_BILLING_SCHEDULE:
        ...
        return update_billing_schedule(
            user, workspace, payload["schedule_id"], payload["data"]
        )
    ...
    if transition is FinancialTransition.REFRESH_INVOICE_LIFECYCLE:
        ...
        return refresh_invoice_lifecycle(
            payload["invoice_id"], workspace, as_of=payload.get("as_of")
        )

    if transition is FinancialTransition.GENERATE_LATE_FEE:
        from .late_fee_service import generate_late_fee
        return generate_late_fee(
            user,
            workspace,
            payload["invoice_id"],
