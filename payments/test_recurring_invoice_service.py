...
class RecurringInvoiceGenerationTests(TestCase):
    ...
    def test_direct_schedule_linked_charge_is_rejected(self):
        with self.assertRaisesMessage(
            ValidationError,
            "Schedule-linked charges must be generated by the canonical recurring billing service",
        ):
            create_charge(
                self.owner,
                self.workspace,
    ...
    @patch("payments.recurring_billing_service._create_charge_record")
