...
def _sum_adjustments(invoice):
    ...
    credit = sum((totals.get(FinancialAdjustment.TYPE_CREDIT, Decimal("0")), totals.get(FinancialAdjustment.TYPE_DISCOUNT, Decimal("0")), totals.get(FinancialAdjustment.TYPE_WAIVER, Decimal("0")), totals.get(FinancialAdjustment.TYPE_WRITE_OFF, Decimal("0"))), Decimal("0"))
    return debit, credit, totals


def _sum_late_fees(invoice):
    from .late_fee_models import LateFee
    return LateFee.objects.filter(invoice=invoice).aggregate(total=Sum("amount"))["total"] or Decimal("0")
...
def _same_idempotent_operation(existing, *, invoice, adjustment_type, amount, reason, reference):
    return existing.invoice_id == invoice.id and existing.adjustment_type == adjustment_type and existing.amount == amount and existing.reason == reason and existing.reference == reference
...
def create_financial_adjustment(user, workspace, data):
    ...
    with transaction.atomic():
        ...
        if idempotency_key:
            existing = FinancialAdjustment.objects.filter(workspace=workspace, idempotency_key=idempotency_key).first()
            if existing:
                if not _same_idempotent_operation(existing, invoice=invoice, adjustment_type=adjustment_type, amount=amount, reason=reason.strip(), reference=reference):
        ...
        reducing_types = {FinancialAdjustment.TYPE_CREDIT, FinancialAdjustment.TYPE_DISCOUNT, FinancialAdjustment.TYPE_WAIVER, FinancialAdjustment.TYPE_WRITE_OFF}
