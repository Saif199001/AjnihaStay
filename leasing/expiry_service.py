...
def expire_lease(user, workspace, lease_id, *, effective_date=None):
    ...
    with transaction.atomic():
        ...
        if lease.status == Lease.STATUS_EXPIRED:
            event = LeaseLifecycleEvent.objects.filter(lease=lease, event_key=LeaseLifecycleEvent.EVENT_EXPIRED).first()
        ...
        if lease.status != Lease.STATUS_ACTIVE:
            raise ValidationError(f"Only active leases can expire; current status is {lease.status}")
        ...
        if effective_date is not None and effective_date != expiry_date:
            raise ValidationError("Lease expiry effective date must match the active contract end date")
        now = timezone.now()
        ...
        append_lifecycle_event(lease=lease, event_type=LeaseLifecycleEvent.EVENT_EXPIRED, actor=user,
                               occurred_at=now, effective_date=expiry_date,
                               metadata={"from_status": previous_status, "to_status": Lease.STATUS_EXPIRED,
