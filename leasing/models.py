...
class Lease(models.Model):
    ...
    STATUS_CHOICES = ((STATUS_DRAFT, "Draft"), (STATUS_PENDING_SIGNATURE, "Pending Signature"), (STATUS_ACTIVE, "Active"), (STATUS_EXPIRED, "Expired"), (STATUS_TERMINATED, "Terminated"), (STATUS_CANCELLED, "Cancelled"))
    VALID_STATUSES = {choice[0] for choice in STATUS_CHOICES}

    workspace = models.ForeignKey("workspaces.Workspace", on_delete=models.PROTECT, related_name="leases")
    occupancy = models.OneToOneField("tenant.Occupancy", on_delete=models.PROTECT, related_name="lease")
    ...
    updated_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="leases_updated", null=True, blank=True)
    ...
    class Meta:
        indexes = [models.Index(fields=["workspace", "status"], name="lease_ws_status_idx"), models.Index(fields=["workspace", "start_date"], name="lease_ws_start_idx"), models.Index(fields=["workspace", "end_date"], name="lease_ws_end_idx")]
        constraints = [
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk and not allow_lifecycle_mutation:
            ...
            if previous_status is not None and previous_status != self.status:
                raise ValidationError("Lease lifecycle status changes must use the canonical lifecycle service")
        self.clean()
