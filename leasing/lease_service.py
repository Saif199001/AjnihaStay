...
MUTABLE_FIELDS = {
    "agreement_number", "start_date", "end_date", "rent_amount", "security_deposit",
    "notice_period_days", "terms", "agreement_reference",
}
IMMUTABLE_CONTRACT_STATUSES = {Lease.STATUS_ACTIVE, Lease.STATUS_EXPIRED, Lease.STATUS_TERMINATED, Lease.STATUS_CANCELLED}
ALLOWED_TRANSITIONS = {
    Lease.STATUS_DRAFT: {Lease.STATUS_PENDING_SIGNATURE},
    Lease.STATUS_PENDING_SIGNATURE: {Lease.STATUS_ACTIVE},
    Lease.STATUS_ACTIVE: set(), Lease.STATUS_EXPIRED: set(), Lease.STATUS_TERMINATED: set(), Lease.STATUS_CANCELLED: set(),
}
STATUS_EVENT_TYPES = {
...
def _get_locked_occupancy(occupancy_id, workspace):
    try:
        return Occupancy.objects.select_for_update().select_related("tenant").get(id=occupancy_id, tenant__workspace=workspace)
...
def _get_locked_lease(lease_id, workspace):
    try:
        return Lease.objects.select_for_update().select_related("occupancy__tenant").get(id=lease_id, workspace=workspace)
...
def _ensure_active_contract_version(lease):
    ...
    return LeaseContractVersion.objects.create(
        lease=lease, workspace=lease.workspace, version_number=1,
        start_date=lease.start_date, end_date=lease.end_date,
        rent_amount=lease.rent_amount, security_deposit=lease.security_deposit,
        notice_period_days=lease.notice_period_days, terms=dict(lease.terms or {}),
        agreement_reference=lease.agreement_reference, created_by=lease.created_by,
    )
...
def create_lease(user, workspace, data):
    ...
    with transaction.atomic():
        ...
        security_deposit = _decimal(data.get("security_deposit", occupancy.security_deposit or 0), "Security deposit")
        allowed = {"agreement_number", "start_date", "end_date", "rent_amount", "security_deposit", "notice_period_days", "status", "terms", "agreement_reference"}
        ...
        lease = Lease(
            workspace=workspace, occupancy=occupancy, agreement_number=data.get("agreement_number", ""),
            start_date=start_date, end_date=end_date, rent_amount=rent_amount, security_deposit=security_deposit,
            notice_period_days=data.get("notice_period_days", 0), status=Lease.STATUS_DRAFT,
            terms=data.get("terms") or {}, agreement_reference=data.get("agreement_reference", ""), created_by=user,
        )
        lease.save()
        append_lifecycle_event(lease=lease, event_type=LeaseLifecycleEvent.EVENT_CREATED, actor=user,
                               occurred_at=lease.created_at, effective_date=lease.start_date,
                               metadata={"status": Lease.STATUS_DRAFT}, event_key=LeaseLifecycleEvent.EVENT_CREATED)
        return lease
...
def transition_lease(user, workspace, lease_id, target_status):
    ...
    if target_status in {Lease.STATUS_EXPIRED, Lease.STATUS_TERMINATED}:
        raise ValidationError("Terminal lease transitions must use their dedicated lifecycle service")
    with transaction.atomic():
        lease = _get_locked_lease(lease_id, workspace)
        ...
        if target_status not in allowed:
            raise ValidationError(f"Invalid lease transition: {previous_status} -> {target_status}")
        ...
        append_lifecycle_event(lease=lease, event_type=STATUS_EVENT_TYPES[target_status], actor=user,
                               occurred_at=now, effective_date=now.date(),
