...
def terminate_lease(user, workspace, lease_id, *, reason, effective_date=None):
    ...
    if len(normalized_reason) > MAX_TERMINATION_REASON_LENGTH:
        raise ValidationError(f"Termination reason cannot exceed {MAX_TERMINATION_REASON_LENGTH} characters")
    ...
    with transaction.atomic():
        ...
        if lease.status == Lease.STATUS_TERMINATED:
            event = LeaseLifecycleEvent.objects.filter(lease=lease, event_key=LeaseLifecycleEvent.EVENT_TERMINATED).first()
            if event and (event.effective_date != requested_date or event.metadata.get("reason") != normalized_reason):
                raise ValidationError("Lease is already terminated with immutable termination details")
            return lease
        if lease.status != Lease.STATUS_ACTIVE:
            raise ValidationError(f"Only active leases can terminate; current status is {lease.status}")
        ...
        latest_version = type(contract_version).objects.select_for_update().filter(lease=lease).order_by("-version_number").first()
        ...
        append_lifecycle_event(lease=lease, event_type=LeaseLifecycleEvent.EVENT_TERMINATED, actor=user,
                               occurred_at=now, effective_date=requested_date,
                               metadata={"from_status": previous_status, "to_status": Lease.STATUS_TERMINATED,
                                         "reason": normalized_reason, "contract_version_id": latest_version.id,
