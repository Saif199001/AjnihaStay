...
class LeaseServiceHardeningTests(TestCase):
    def setUp(self):
        ...
        self.property = Property.objects.create(owner=self.owner, workspace=self.workspace, name="P13 Property", property_type="flat", address="Address", city="Lucknow", state="UP", pincode="226001")
        self.unit = Unit.objects.create(property=self.property, unit_type="flat", unit_number="101", rent=Decimal("12000.00"))
        self.tenant = Tenant.objects.create(owner=self.owner, workspace=self.workspace, full_name="P13 Tenant", phone="9999999999", email="p13-hard-tenant@example.com", permanent_address="Lucknow, Uttar Pradesh")
    ...
    def data(self, **overrides):
        value = {"occupancy": self.occupancy, "start_date": date(2026, 1, 1), "end_date": date(2026, 12, 31), "rent_amount": Decimal("12000.00"), "security_deposit": Decimal("24000.00")}
    ...
    def test_direct_model_status_mutation_is_rejected(self):
        lease = create_lease(self.manager, self.workspace, self.data())
        lease.status = Lease.STATUS_PENDING_SIGNATURE
        with self.assertRaises(ValidationError):
            lease.save()
        lease.refresh_from_db()
        self.assertEqual(lease.status, Lease.STATUS_DRAFT)

    def test_bulk_status_mutation_is_rejected(self):
        lease = create_lease(self.manager, self.workspace, self.data())
        with self.assertRaises(ValidationError):
            Lease.objects.filter(pk=lease.pk).update(status=Lease.STATUS_PENDING_SIGNATURE)
        lease.refresh_from_db()
        self.assertEqual(lease.status, Lease.STATUS_DRAFT)
    ...
    def test_lifecycle_terminal_state_is_irreversible(self):
        ...
        terminate_lease(self.manager, self.workspace, lease.id, reason="P1.3 hardening test", effective_date=date.today())
        lease.refresh_from_db()
    ...
    def test_database_unique_occupancy_contract_remains_enforced(self):
        Lease.objects.create(workspace=self.workspace, occupancy=self.occupancy, start_date=date(2026, 1, 1), end_date=date(2026, 12, 31), rent_amount=Decimal("12000.00"), security_deposit=Decimal("24000.00"), created_by=self.manager)
        with self.assertRaises(IntegrityError):
            with transaction.atomic():
                Lease.objects.create(workspace=self.workspace, occupancy=self.occupancy, start_date=date(2027, 1, 1), end_date=date(2027, 12, 31), rent_amount=Decimal("12000.00"), security_deposit=Decimal("24000.00"), created_by=self.manager)
