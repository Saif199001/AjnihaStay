...
def cancel_lease(
    ...
    *,
    ...
    """Cancel a pre-activation Lease through the canonical mutation boundary.

    Cancellation is distinct from termination: only draft/pending-signature
    leases may be cancelled. The operation changes only Lease lifecycle state
    and immutable history; Occupancy and financial truth are untouched.
    """
    ...
    with transaction.atomic():
        ...
        if lease.status not in {
            ...
            raise ValidationError(
                "Only draft or pending-signature leases can be cancelled"
            )
        ...
        append_lifecycle_event(
            lease=lease,
            event_type=LeaseLifecycleEvent.EVENT_CANCELLED,
            actor=user,
            occurred_at=now,
            effective_date=effective_date,
            metadata={
                "from_status": previous_status,
                "to_status": Lease.STATUS_CANCELLED,
